...
def dict_to_list(value):
    ...
    Args:
        value: Строка с разделителями или None
    Returns:
        None если входное значение пустое, иначе список строк
    """
...
def state_story_transform(state):
    """
    Преобразует статус задачи из формата Azure в формат Yandex.Tracker
    Args:
        state: Статус задачи в Azure
    Returns:
        Соответствующий статус в Yandex.Tracker
    """
    state_mapping = {
        "new": "new",
        "reopened": "reopened",
...
def bug_description(attributes):
    """
    Формирует описание бага в HTML формате
    Args:
        attributes: Словарь с атрибутами задачи
    Returns:
        Отформатированное HTML описание бага
    """
    sections = {
...
def migration(token, org_id, attributes, title, type):
    """
    Создает задачу в Yandex.Tracker через API
    Args:
        token: API токен
...
def excel_parse(token, org_id, type):
    """
    Читает данные из CSV файла и запускает миграцию задач
    Args:
        token: API токен
        org_id: ID организации
        type: Тип задач для миграции
    """
    # Маппинг типов задач и соответствующих полей заголовков
    work_item_mapping = {
    ...
    # Создание прогресс-бара для отслеживания процесса
    with tqdm(total=total_items, desc=f"Миграция {type}", unit="задача") as pbar:
        for _, row in filtered_df.iterrows():
            # Формирование словаря атрибутов из строки датафрейма
            attributes = {
                'id': str(row['ID']),
            ...
            # Запуск миграции для текущей задачи
...
def summarize_log(log_file='migration.log'):
    """
    Анализирует лог-файл и формирует статистику по миграции
    Args:
...
if __name__ == "__main__":
    ...
    Обрабатывает аргументы командной строки и запускает соответствующие операции
    """
    try:
        ...
        # Выполнение операции в зависимости от аргументов
        if args.log == 'final':
            summarize_log()
        else:
